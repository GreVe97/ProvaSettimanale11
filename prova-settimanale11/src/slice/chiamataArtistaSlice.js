...
export const getArtistaInfo = createAsyncThunk("chiamataArtistaInfo/fetch", async (id) => {
    console.log(id);
    return axios(urlArtista + id.id)
        .then((response) => { console.log(response.data); return response.data })
})

export const getArtistaTracks = createAsyncThunk("chiamataArtistaTracks/fetch", async (url) => {
    console.log(url);
    return axios(url)
        .then((response) => { console.log(response.data); return response.data })
})
...
const chiamataArtista_slice = createSlice(
    {
        ...
        extraReducers: builder => {
            builder.addCase(getArtistaTracks.pending, (state, action) => {
                state.loading = true;
